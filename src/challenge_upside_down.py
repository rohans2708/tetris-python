"""
Challenge: Kopfstand (Upside Down)

- Das Spielfeld wird vertikal gespiegelt gerendert.
- Die Rotationsrichtung ist invertiert (CW/CCW vertauscht).
- Physik (Kollision, Linien-Clear etc.) bleibt unverändert – nur Darstellung/Rotate-Eingabe.
"""
...
class Challenge_Upside_Down(MainBoard):
    def __init__(self, starting_level: int, score: int, upgrades=None):
        # Upgrades und Ghost-Option an das Basisspielfeld weiterreichen
        super().__init__(starting_level, score, upgrades)
        self.updateSpeed()

    # ---- Hilfsfunktion: Zeilenindex vertikal spiegeln ----
    def _flip_row(self, row: int) -> int:
        return (self.rowNum - 1) - row

    # ---- Rotationen invertieren: CW<->CCW ----
    def rotate_CC(self):
        super().rotate_cCC()
    ...
    def draw_GAMEBOARD_CONTENT(self):
        ...
        # --- bewegendes Piece + Ghost invertiert zeichnen ---
        ...
        # --- Grid-Linien wie im Original (passen zu invertierten Zellen) ---
        ...
        # --- Pause/GameOver Overlays unverändert ---
        if self.gamePause:
            import pygame
